...
public:
    ...
    part_view_t mis_2(const matrix_t& g) {

        ordinal_t n = g.numRows();

        part_view_t state("is membership", n);

        ordinal_t unassigned_total = n;
        rand_view_t randoms("randomized", n);
        ...
        while (unassigned_total > 0) {
            ...
            unassigned_total = 0;
            Kokkos::parallel_reduce(policy_t(0, n), KOKKOS_LAMBDA(const ordinal_t i, ordinal_t & thread_sum){
                if (state(i) == 0) {
                    if (tuple_idx(i) == i) {
                        state(i) = 1;
                    }
    ...
    coarse_map coarsen_mis_2(const matrix_t& g) {

        typename matrix_t::staticcrsgraph_type::entries_type::non_const_value_type nc = 0;
        vtx_view_t vcmap = KokkosGraph::graph_mis2_coarsen<Device, typename matrix_t::staticcrsgraph_type::row_map_type, typename matrix_t::staticcrsgraph_type::entries_type, vtx_view_t>(g.graph.row_map, g.graph.entries, nc);
    ...
    coarse_map coarsen_GOSH(const matrix_t& g) {
    ...
    coarse_map coarsen_GOSH_v2(const matrix_t& g) {
    ...
    int recoarsen_HEC(matrix_t& interp,
        ...
        timer.reset();
        ordinal_t nc = parallel_map_construct(vcmap, n, vperm, hn);
        timer.reset();
    ...
    int recoarsen_match(matrix_t& interp,
        ordinal_t* nvertices_coarse_ptr,
        const matrix_t& g,
        const part_view_t part,
        ExperimentLoggerUtil<scalar_t>& experiment) {
        ...
        if (false && unmappedRatio > 0.25) {
                ...
                Kokkos::View<uint64_t*, Device> htable(Kokkos::ViewAllocateWithoutInitializing("hashes hash table"), unmapped);
                vtx_view_t twins(Kokkos::ViewAllocateWithoutInitializing("twin table"), unmapped);
                Kokkos::deep_copy(htable, 0);
                Kokkos::deep_copy(twins, -1);
                edge_subview_t t_twins("total twins");
                Kokkos::parallel_for("match by hash", policy_t(0, unmapped), KOKKOS_LAMBDA(const ordinal_t x){
                    ordinal_t i = unmappedVtx(x);
                    uint64_t h = hashes(x);
                    //0 is the null val
                    if(h == 0) h = 1;
                    ordinal_t key = h % static_cast<uint64_t>(unmapped);
                    bool found = false;
    ...
    coarse_map coarsen_match(const matrix_t& g,
        ...
        while (perm_length > 0) {
            ...
            if(uniform_weights){
                ...
                if(g.nnz() / g.numRows() > 32){
                    Kokkos::parallel_for("Heaviest HN", team_policy_t(perm_length, Kokkos::AUTO), KOKKOS_LAMBDA(const member& thread) {
                        const ordinal_t i = thread.league_rank();
                        ordinal_t u = perm_length == n ? i : vperm(i);
                        if(vcmap(u) != ORD_MAX) return;
                        scalar_t max_ewt = 0;
        ...
        if (match_choice == 1) {
                            ...
                            if (vcmap(v) == ORD_MAX) {
                                ...
                                if (g.graph.row_map(v + 1) - g.graph.row_map(v) == 1) {
                                    if (lastLeaf == ORD_MAX) {
                                        ...
                                        ordinal_t cv = lastLeaf < v ? lastLeaf : v;
                                        vcmap(lastLeaf) = cv;
                                        vcmap(v) = cv;
                                        lastLeaf = ORD_MAX;
                                    }
            ...
            if (unmappedRatio > 0.25) {
                ...
                vtx_view_t relative_bit("vertices with unmapped relatives", n);
                Kokkos::parallel_for("set mappable relatives", policy_t(0, unmapped), KOKKOS_LAMBDA(const ordinal_t x){
                    ordinal_t i = mappableVtx(x);
                    for(edge_offset_t j = g.graph.row_map(i); j < g.graph.row_map(i + 1); j++){
                        ordinal_t v = g.graph.entries(j);
                        if(vcmap(v) != ORD_MAX) relative_bit(v) = 1;
                    }
                });
                ordinal_t degree_limit = (g.nnz() / g.numRows()) * 100;
                vtx_view_t relatives("vertices with unmapped relatives", n);
                ordinal_t relative_count = 0;
                Kokkos::parallel_scan("get unmapped", policy_t(0, n), KOKKOS_LAMBDA(const ordinal_t i, ordinal_t & update, const bool final){
                    ordinal_t degree = g.graph.row_map(i+1) - g.graph.row_map(i);
                    if (relative_bit(i) == 1 && degree <= degree_limit) {
                        if (final) {
                            relatives(update) = i;
                            relative_bit(i) = 0;
                        }
                ...
                do {
                    ...
                    //printf("mappable count: %u; relative count: %u\n", mappable_count, relative_count);
                    //choose relatives for unmapped vertices
                    Kokkos::parallel_for("assign relatives", policy_t(0, relative_count), KOKKOS_LAMBDA(ordinal_t x){
                        ordinal_t i = relatives(x);
                        if (vcmap(i) != ORD_MAX) {
                            ordinal_t last_free = ORD_MAX;
                            for (edge_offset_t j = g.graph.row_map(i); j < g.graph.row_map(i + 1); j++) {
                                ordinal_t v = g.graph.entries(j);
                    ...
                    ordinal_t old_mappable = mappable_count;
                    vtx_view_t nextMappable("next mappable vertices", old_mappable);
                    vtx_view_t nextRelatives("next relatives", relative_count);
                    ...
                    mappableVtx = nextMappable;
                    Kokkos::parallel_for("set mappable relatives", policy_t(0, mappable_count), KOKKOS_LAMBDA(const ordinal_t x){
                        ordinal_t i = mappableVtx(x);
                        for(edge_offset_t j = g.graph.row_map(i); j < g.graph.row_map(i + 1); j++){
                            ordinal_t v = g.graph.entries(j);
                            if(vcmap(v) != ORD_MAX) relative_bit(v) = 1;
                    ...
                    Kokkos::parallel_scan("get next relatives", policy_t(0, relative_count), KOKKOS_LAMBDA(const ordinal_t i, ordinal_t & update, const bool final){
                        ordinal_t u = relatives(i);
                        ordinal_t degree = g.graph.row_map(u+1) - g.graph.row_map(u);
                        if (relative_bit(u) == 1 && degree < degree_limit) {
                            if (final) {
                                nextRelatives(update) = u;
                                relative_bit(u) = 0;
                            }
